...
# +----------++-------------------------+------------+-----+------++------------------+
# |          ||                         |            |offset2     ||                  |
# |          || phrase_offsets[]        | magic      |     |frequency                 |
...
class MschxudpBuilder(object):
    ...
    def add_phrase(self, shortcut: str, phrase: str, candidate: int = 0, frequency: int = 6):
        if candidate == 0:
            candidate = sum(1 for item in self.phrases if item["shortcut"] == shortcut) + 1
        self.phrases.append({
            "shortcut": shortcut,
            "phrase": phrase,
            "candidate": candidate,
